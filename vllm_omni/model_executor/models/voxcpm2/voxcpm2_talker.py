...
def _device_to_native_arg(device: torch.device) -> str | None:
    """Return a native VoxCPM device string when it supports the backend."""
    if device.type == "cuda":
    ...
    if device.type == "cpu":
        return "cpu"
    # Older voxcpm packages only validate cuda/mps/cpu.  For other backends
    # instantiate on CPU, then move the assembled modules with PyTorch .to().
    return "cpu"
...
def build_voxcpm2_prompt(
    ...
    return {
        "prompt_token_ids": [1] * prefill_len,
...
class VoxCPM2TalkerForConditionalGeneration(nn.Module):
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        native = VoxCPM.from_pretrained(
            ...
            load_denoiser=False,
            optimize=False,
            device=_device_to_native_arg(self._device),
        )
        self._tts: nn.Module = native.tts_model.to(self._device)
        self._tts.device = str(self._device)
        self._tts.config.device = str(self._device)
