...
def test_build_multistage_generation_inputs_hunyuan_image3_single_edit_uses_image_modality_from_model_name(
    serving_chat,
):
    from vllm_omni.entrypoints.openai.serving_chat import OmniOpenAIServingChat

    serving_chat._diffusion_model_name = "tencent/HunyuanImage-3.0-Instruct"
    engine = SimpleNamespace(
        stage_configs=[
            SimpleNamespace(stage_type="llm", is_comprehension=True),
            SimpleNamespace(stage_type="diffusion", is_comprehension=False),
...
def test_build_multistage_generation_inputs_hunyuan_image3_single_edit_uses_image_modality_from_config_path(
    serving_chat,
):
    from vllm_omni.entrypoints.openai.serving_chat import OmniOpenAIServingChat

    engine = SimpleNamespace(
        config_path="vllm_omni/deploy/hunyuan_image3_it2i_npu_aclgraph_rope_vitdp.yaml",
        stage_configs=[
            SimpleNamespace(stage_type="llm", is_comprehension=True),
            SimpleNamespace(stage_type="diffusion", is_comprehension=False),
        ],
